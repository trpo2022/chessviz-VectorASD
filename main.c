...
struct Step parser(text str) {
    ...
    while (str[pos] && R < 5) {
        ...
        case 'h':
            if (R == 0 || R == 1) {
                if (step.x)
                    return error_parser_const_pool("Вы уже ввели букву клетки ходящей "
                                                   "фигуры или её "
                                                   "тип");
                step.x = let;
                R = 1;
                ...
                if (step.x2)
                    return error_parser_const_pool("Вы уже ввели букву клетки, куда ходит "
                                                   "фигура");
                step.x2 = let;
            } else
                return error_parser_generated(R, let);
        ...
        case '8':
            if (R == 0 || R == 1) {
                if (!step.x)
                    return error_parser_const_pool("Вы ещё не ввели букву клетки ходящей "
                                                   "фигуры или "
                                                   "её тип");
                step.y = let;
                R = 2;
                ...
                if (!step.x2)
                    return error_parser_const_pool("Вы ещё не ввели букву клетки, куда "
                                                   "ходит фигура");
                step.y2 = let;
                R = 4;
            } else
...
text caserson(char s, char p) {
    text figures[6][4] = {{"p", "пешка", "пешку", "пешкой"}, {"r", "ладья", "ладью", "ладьёй"}, {"n", "конь", "коня", "конём"}, {"b", "слон", "слона", "слоном"}, {"q", "ферзь", "ферзя", "ферзем"}, {"k", "король", "короля", "королём"}};
    s = tolower(s);
    for (int i = 0; i < 6; i++)
...
void error_handler_const_pool(text err) { printf("Ошибка игровой механики:\n  %s\n", err); }
...
void handler(char board[8][8], struct Step step, struct Vector *history, int *step_n) {
    ...
    if (tile == ' ') return error_handler_const_pool("Вы не можете ходить пустой клеткой");
    if (x == x2 && y == y2) return error_handler_const_pool("Вы не можете ходить в ту же самую клетку");
    if (is_white && !tile_is_white) return error_handler_const_pool("Вы не можете ходить чёрными фигурами");
    if ((!is_white) && tile_is_white) return error_handler_const_pool("Вы не можете ходить белыми фигурами");
    if (tile2 == ' ') {
        if (step.type == 'x') return error_handler_const_pool("Вы не можете ходить в режиме 'x'. Используйте '-'");
    } else {
        if (is_white == tile2_is_white) return error_handler_const_pool("Вы не можете рубить свои же фигуры");
        if (step.type == '-') return error_handler_const_pool("Вы не можете рубить в режиме '-'. Используйте 'x'");
    }
    if (step.figure && step.figure != toupper(tile)) return error_handler_caserson("Вы хотели походить %s '%c', но на поле оказалась %s '%c'", step.figure, tile);
    if (step.figure2 && step.type == '-')
        return error_handler_const_pool("Вы не можете выбирать, кого хочете рубить, в режиме '-'. "
                                        "Используйте 'x'");
    ...
    case 'p':
        if (step.type == '-') {
            if ((dx != 0 || dy != -1) && is_white)
                return error_handler_const_pool("Белая пешка может ходить только вверх на одну "
                                                "клетку");
            if ((dx != 0 || dy != 1) && !is_white)
                return error_handler_const_pool("Чёрная пешка может ходить только вниз на одну "
                                                "клетку");
        } else {
            if ((dy != -1 || (dx != -1 && dx != 1)) && is_white)
                return error_handler_const_pool("Белая пешка может рубить только вверх по "
                                                "диагонали на "
                                                "одну клетку");
            if ((dy != 1 || (dx != -1 && dx != 1)) && !is_white)
                return error_handler_const_pool("Чёрная пешка может рубить только вниз по "
                                                "диагонали на "
                                                "одну клетку");
        }
        break;
    case 'r':
        if (dx == 0) {
            ...
            return error_handler_const_pool("Тура не может перемещаться по диагонали");
    }
